...
// Components
function AgentCard({ title, author, description, avatars, tabId, onClick }: AgentCardProps) {
  const handleClick = () => {
    if (onClick) {
      onClick();
    }
...
const agentCards = [
  ...
  },
  {
    title: "Trading Bot",
    author: "@tregodefai",
    description: "Automated trading bot that monitors social media and market signals to execute trades on Aptos chain. Configure your trading parameters and let the bot handle token purchases based on your strategy.",
    avatars: [
...
const stats = [
  {
    ...
    icons: [
      ...
      { color: "bg-blue-600", symbol: "◆" },
      { color: "bg-green-500", symbol: "☘" },
      { color: "bg-yellow-500", symbol: "K" },
      { color: "bg-amber-600", symbol: "📦" },
      { color: "bg-cyan-600", symbol: "◈" },
      { color: "bg-purple-600", symbol: "≡" },
    ],
  },
  {
    count: "100+",
  ...
  },
  {
    ...
    label: "dApps",
    icons: [
      { color: "bg-green-600", symbol: "🌿" },
      { color: "bg-pink-500", symbol: "🦄" },
      { color: "bg-purple-600", symbol: "👻" },
      { color: "bg-cyan-500", symbol: "◉" },
...
export function DashboardContent({ onTabChange }: DashboardContentProps) {
  return (
          ...
          <div className="w-full max-w-md space-y-3 sm:space-y-4">
            ...
            {agentCards.map((card, index) => (
              <AgentCard
                key={index}
                {...card}
                onClick={() => card.tabId && onTabChange(card.tabId)}
              />
            ))}
          </div>
        </div>
